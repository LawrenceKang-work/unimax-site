...
/* 2026-08-28 新增:「how to become a supplement distributor」步骤章节(SERP 获胜形态 = 编号步骤正文) */
const OL_STEPS_EN = `      <ol>
        <li><strong>Pick the category you can actually sell.</strong> Start from your channels, not the product: the shops, gyms, clinics or online audience you already reach. A distributor with warm channels beats one with a full warehouse.</li>
        <li><strong>Check your market's paperwork before ordering.</strong> Most countries have no specific "supplement distributor licence" — you operate under a normal business registration — but import rules and product notification requirements vary by market. Confirm both before your first order, not after.</li>
        <li><strong>Shortlist brands and verify their certifications.</strong> Ask every candidate brand for its manufacturing certificates (HALAL, GMP, HACCP or your market's equivalent) and check the certificate holder matches the factory that actually makes the product. Our <a href="/guides/tongkat-ali-drink-supplier-checklist/">supplier checklist</a> covers the exact questions to ask.</li>
        <li><strong>Agree starter terms in writing.</strong> Minimum order, price structure, territory, and what marketing support the brand provides. A serious brand answers these directly — with UNI MAX, this conversation happens over WhatsApp with a same-day reply.</li>
        <li><strong>Start with a shelf trial, scale on data.</strong> Order the smallest sensible quantity — UNI MAX suggests 12–24 boxes for a first retail position — place it in your best channel, and let sell-through decide your second order, not optimism.</li>
      </ol>`;

const H2_STEPS_EN = 'How to become a supplement distributor: five steps';
const LEAD_STEPS_EN = 'If distribution is the route you are leaning towards, the path from reading this page to stock on a shelf is shorter than most guides make it sound. The five steps are the same wherever you are based — only the paperwork details change by market.';
...
zh: {
  ...
  faq: [
    ...
    { q: '卖补剂需要执照吗？', a: '在大多数市场，并没有专门的"补剂分销商执照"——你在正常的营业登记下销售成品补剂。真正因国家而异的是进口手续，以及成品在销售前是否需要向当地主管部门备案或注册。实际的做法是：先让品牌方把认证与产品文件拿出来——对 UNI MAX 来说就是生产工厂的证书（HALAL、GMP、MeSTI、ISO 9001、HACCP，登记在 Orient Biotech Sdn Bhd 名下）——然后在第一笔订单之前，向你所在国家的主管部门确认进口与备案要求。' },
  ],
  html: {
    'Business Guide': '商业指南',
    ...
    [H2_STEPS_EN]: '怎么成为补剂分销商：五个步骤',
    [LEAD_STEPS_EN]: '如果你倾向的是分销这条路，从读完这页到货真正摆上货架，其实比大多数指南写的要短。这五个步骤在哪个市场都一样——因市场而异的只是手续细节。',
    [OL_STEPS_EN]: `      <ol>
        <li><strong>选一个你真的卖得动的品类。</strong>从你的渠道出发，而不是从产品出发：你已经触达的门店、健身房、诊所或线上受众。手里有热渠道的分销商，胜过只有一仓库货的分销商。</li>
        <li><strong>下单前先查清你所在市场的手续。</strong>大多数国家并没有专门的"补剂分销商执照"——你在正常的营业登记下经营——但进口规则和产品备案要求因市场而异。两样都要在第一笔订单之前确认，不是之后。</li>
        <li><strong>筛选品牌并核实认证。</strong>让每个候选品牌出示生产认证（HALAL、GMP、HACCP 或你所在市场的对应认证），并核对证书持有方是否就是实际生产该产品的工厂。我们的<a href="/guides/tongkat-ali-drink-supplier-checklist/">供应商甄选清单</a>列出了要问的具体问题。</li>
        <li><strong>把起步条款落到书面。</strong>起订量、价格结构、区域，以及品牌方提供哪些营销支持。认真的品牌会直接回答这些问题——UNI MAX 的这场对话通过 WhatsApp 进行，当天回复。</li>
        <li><strong>从货架试销开始，凭数据加码。</strong>先订一个合理范围内最小的量——UNI MAX 建议第一个零售点位 12–24 盒——摆进你最好的渠道，让动销数据决定你的第二笔订单，而不是让乐观情绪决定。</li>
      </ol>`,
...
ms: {
  ...
  faq: [
    ...
    { q: 'Adakah saya perlukan lesen untuk menjual suplemen?', a: 'Di kebanyakan pasaran tiada "lesen pengedar suplemen" yang khusus — anda menjual suplemen siap di bawah pendaftaran perniagaan biasa. Yang berbeza mengikut negara ialah urusan kertas kerja import dan sama ada produk siap perlu dinotifikasikan atau didaftarkan dengan pihak berkuasa tempatan sebelum dijual. Langkah praktikalnya: minta jenama menyediakan pensijilan dan dokumentasi produk terlebih dahulu — dengan UNI MAX ini bermaksud sijil kemudahan pembuatan (HALAL, GMP, MeSTI, ISO 9001, HACCP, didaftarkan kepada Orient Biotech Sdn Bhd) — kemudian sahkan keperluan import dan notifikasi negara anda dengan pihak berkuasa tempatan sebelum pesanan pertama anda.' },
  ],
  html: {
    ...
    [H2_STEPS_EN]: 'Cara menjadi pengedar suplemen: lima langkah',
    [LEAD_STEPS_EN]: 'Jika pengedaran ialah laluan yang anda cenderungi, perjalanan daripada membaca halaman ini hingga stok berada di rak sebenarnya lebih pendek daripada yang digambarkan kebanyakan panduan. Lima langkah ini sama di mana sahaja anda berada — hanya butiran urusan kertas kerja yang berbeza mengikut pasaran.',
    [OL_STEPS_EN]: `      <ol>
        <li><strong>Pilih kategori yang anda benar-benar boleh jual.</strong> Mulakan daripada saluran anda, bukan produk: kedai, gim, klinik atau audiens dalam talian yang sudah anda capai. Pengedar dengan saluran sedia panas mengatasi pengedar yang hanya ada gudang penuh.</li>
        <li><strong>Semak urusan kertas kerja pasaran anda sebelum membuat pesanan.</strong> Kebanyakan negara tiada "lesen pengedar suplemen" yang khusus — anda beroperasi di bawah pendaftaran perniagaan biasa — tetapi peraturan import dan keperluan notifikasi produk berbeza mengikut pasaran. Sahkan kedua-duanya sebelum pesanan pertama anda, bukan selepasnya.</li>
        <li><strong>Senarai pendek jenama dan sahkan pensijilan mereka.</strong> Minta setiap jenama calon menunjukkan sijil pembuatan (HALAL, GMP, HACCP atau setara di pasaran anda) dan semak pemegang sijil sepadan dengan kilang yang benar-benar menghasilkan produk itu. <a href="/guides/tongkat-ali-drink-supplier-checklist/">Senarai semak pembekal</a> kami merangkumi soalan tepat untuk ditanya.</li>
        <li><strong>Persetujui terma permulaan secara bertulis.</strong> Pesanan minimum, struktur harga, wilayah, dan sokongan pemasaran yang disediakan jenama. Jenama yang serius menjawab semua ini secara terus — dengan UNI MAX, perbualan ini berlaku melalui WhatsApp dengan balasan hari yang sama.</li>
        <li><strong>Mulakan dengan percubaan rak, kembangkan berdasarkan data.</strong> Pesan kuantiti munasabah yang paling kecil — UNI MAX mencadangkan 12–24 kotak untuk posisi runcit pertama — letakkan di saluran terbaik anda, dan biarkan jualan sebenar menentukan pesanan kedua anda, bukan keyakinan semata-mata.</li>
      </ol>`,
...
pl: {
  ...
  faq: [
    ...
    { q: 'Czy potrzebuję licencji, by sprzedawać suplementy?', a: 'Na większości rynków nie ma specjalnej "licencji dystrybutora suplementów" — gotowe suplementy sprzedajesz w ramach zwykłej rejestracji działalności. Tym, co różni się w zależności od kraju, są formalności importowe oraz to, czy gotowe produkty muszą być zgłoszone lub zarejestrowane u lokalnego organu przed sprzedażą. Praktyczny krok: poproś markę o certyfikaty i dokumentację produktu z góry — w przypadku UNI MAX oznacza to certyfikaty zakładu produkcyjnego (HALAL, GMP, MeSTI, ISO 9001, HACCP, zarejestrowane na Orient Biotech Sdn Bhd) — a następnie potwierdź wymogi importowe i zgłoszeniowe swojego kraju u lokalnego organu przed pierwszym zamówieniem.' },
  ],
  html: {
    ...
    [H2_STEPS_EN]: 'Jak zostać dystrybutorem suplementów: pięć kroków',
    [LEAD_STEPS_EN]: 'Jeśli skłaniasz się ku dystrybucji, droga od przeczytania tej strony do towaru na półce jest krótsza, niż sugeruje większość poradników. Te pięć kroków wygląda tak samo niezależnie od tego, gdzie działasz — w zależności od rynku zmieniają się tylko szczegóły formalności.',
    [OL_STEPS_EN]: `      <ol>
        <li><strong>Wybierz kategorię, którą naprawdę potrafisz sprzedawać.</strong> Zacznij od swoich kanałów, nie od produktu: sklepów, siłowni, klinik lub odbiorców online, do których już docierasz. Dystrybutor z ciepłymi kanałami wygrywa z tym, który ma tylko pełny magazyn.</li>
        <li><strong>Sprawdź formalności na swoim rynku przed zamówieniem.</strong> Większość krajów nie ma specjalnej "licencji dystrybutora suplementów" — działasz w ramach zwykłej rejestracji działalności — ale zasady importu i wymogi zgłaszania produktów różnią się w zależności od rynku. Potwierdź jedno i drugie przed pierwszym zamówieniem, nie po nim.</li>
        <li><strong>Stwórz krótką listę marek i zweryfikuj ich certyfikaty.</strong> Poproś każdą markę-kandydata o certyfikaty produkcyjne (HALAL, GMP, HACCP lub odpowiednik na twoim rynku) i sprawdź, czy posiadacz certyfikatu to fabryka, która faktycznie wytwarza produkt. Nasza <a href="/guides/tongkat-ali-drink-supplier-checklist/">lista kontrolna dostawcy</a> zawiera dokładne pytania, które warto zadać.</li>
        <li><strong>Uzgodnij warunki startowe na piśmie.</strong> Minimalne zamówienie, struktura cen, terytorium oraz wsparcie marketingowe, jakie zapewnia marka. Poważna marka odpowiada na te pytania wprost — w przypadku UNI MAX ta rozmowa odbywa się przez WhatsApp z odpowiedzią tego samego dnia.</li>
        <li><strong>Zacznij od testu na półce, skaluj na podstawie danych.</strong> Zamów najmniejszą rozsądną ilość — UNI MAX sugeruje 12–24 pudełka na pierwszą pozycję detaliczną — umieść ją w swoim najlepszym kanale i pozwól, by o drugim zamówieniu zdecydowała faktyczna sprzedaż, a nie optymizm.</li>
      </ol>`,
...
nl: {
  ...
  faq: [
    ...
    { q: 'Heb ik een licentie nodig om supplementen te verkopen?', a: 'In de meeste markten bestaat er geen specifieke "supplementendistributeurslicentie" — je verkoopt kant-en-klare supplementen onder een normale bedrijfsregistratie. Wat wel per land verschilt, is het importpapierwerk en of afgewerkte producten vóór verkoop gemeld of geregistreerd moeten worden bij een lokale autoriteit. De praktische stap: vraag het merk vooraf om zijn certificering en productdocumentatie — bij UNI MAX betekent dit de certificaten van de productiefaciliteit (HALAL, GMP, MeSTI, ISO 9001, HACCP, geregistreerd op Orient Biotech Sdn Bhd) — en bevestig daarna de import- en meldingsvereisten van je eigen land bij je lokale autoriteit vóór je eerste bestelling.' },
  ],
  html: {
    ...
    [H2_STEPS_EN]: 'Hoe word je supplementendistributeur: vijf stappen',
    [LEAD_STEPS_EN]: 'Als distributie de route is waar je naar neigt, is de weg van het lezen van deze pagina naar voorraad op een schap korter dan de meeste gidsen doen vermoeden. De vijf stappen zijn overal hetzelfde — alleen de papierwerkdetails verschillen per markt.',
    [OL_STEPS_EN]: `      <ol>
        <li><strong>Kies de categorie die je echt kunt verkopen.</strong> Begin bij je kanalen, niet bij het product: de winkels, sportscholen, klinieken of online doelgroep die je al bereikt. Een distributeur met warme kanalen wint van een met alleen een vol magazijn.</li>
        <li><strong>Controleer het papierwerk van je markt vóór je bestelt.</strong> De meeste landen kennen geen specifieke "supplementendistributeurslicentie" — je opereert onder een normale bedrijfsregistratie — maar importregels en meldingsvereisten voor producten verschillen per markt. Bevestig beide vóór je eerste bestelling, niet erna.</li>
        <li><strong>Maak een shortlist van merken en verifieer hun certificeringen.</strong> Vraag elk kandidaat-merk om zijn productiecertificaten (HALAL, GMP, HACCP of het equivalent van jouw markt) en controleer of de certificaathouder de fabriek is die het product daadwerkelijk maakt. Onze <a href="/guides/tongkat-ali-drink-supplier-checklist/">leverancierschecklist</a> behandelt precies de vragen die je moet stellen.</li>
        <li><strong>Leg startvoorwaarden schriftelijk vast.</strong> Minimale bestelling, prijsstructuur, territorium, en welke marketingondersteuning het merk biedt. Een serieus merk beantwoordt dit direct — bij UNI MAX verloopt dit gesprek via WhatsApp met een reactie dezelfde dag.</li>
        <li><strong>Begin met een schaptest, schaal op basis van data.</strong> Bestel de kleinste verstandige hoeveelheid — UNI MAX suggereert 12–24 dozen voor een eerste retailpositie — plaats die in je beste kanaal, en laat de doorverkoop je tweede bestelling bepalen, niet optimisme.</li>
      </ol>`,
...
de: {
  ...
  faq: [
    ...
    { q: 'Brauche ich eine Lizenz, um Nahrungsergänzungsmittel zu verkaufen?', a: 'In den meisten Märkten gibt es keine spezielle "Vertriebslizenz für Nahrungsergänzungsmittel" — Sie verkaufen fertige Nahrungsergänzungsmittel unter einer normalen Gewerbeanmeldung. Was je nach Land variiert, sind die Importformalitäten und ob fertige Produkte vor dem Verkauf bei einer lokalen Behörde gemeldet oder registriert werden müssen. Der praktische Schritt: Bitten Sie die Marke vorab um ihre Zertifizierungs- und Produktdokumentation — bei UNI MAX bedeutet das die Zertifikate der Produktionsstätte (HALAL, GMP, MeSTI, ISO 9001, HACCP, registriert auf Orient Biotech Sdn Bhd) — und bestätigen Sie anschließend die Import- und Meldepflichten Ihres Landes bei Ihrer lokalen Behörde vor der ersten Bestellung.' },
  ],
  html: {
    ...
    [H2_STEPS_EN]: 'Wie man Vertriebspartner für Nahrungsergänzungsmittel wird: fünf Schritte',
    [LEAD_STEPS_EN]: 'Wenn Sie zum Vertrieb tendieren, ist der Weg vom Lesen dieser Seite bis zur Ware im Regal kürzer, als die meisten Ratgeber vermuten lassen. Die fünf Schritte sind überall gleich — nur die Details der Formalitäten unterscheiden sich je nach Markt.',
    [OL_STEPS_EN]: `      <ol>
        <li><strong>Wählen Sie die Kategorie, die Sie wirklich verkaufen können.</strong> Beginnen Sie bei Ihren Kanälen, nicht beim Produkt: die Läden, Fitnessstudios, Kliniken oder das Online-Publikum, das Sie bereits erreichen. Ein Vertriebspartner mit warmen Kanälen schlägt einen mit nur einem vollen Lager.</li>
        <li><strong>Prüfen Sie die Formalitäten Ihres Marktes vor der Bestellung.</strong> Die meisten Länder kennen keine spezielle "Vertriebslizenz für Nahrungsergänzungsmittel" — Sie agieren unter einer normalen Gewerbeanmeldung — aber Importregeln und Meldepflichten für Produkte variieren je nach Markt. Bestätigen Sie beides vor Ihrer ersten Bestellung, nicht danach.</li>
        <li><strong>Erstellen Sie eine Auswahlliste von Marken und überprüfen Sie deren Zertifizierungen.</strong> Bitten Sie jede Kandidatenmarke um ihre Herstellungszertifikate (HALAL, GMP, HACCP oder das Äquivalent Ihres Marktes) und prüfen Sie, ob der Zertifikatsinhaber die Fabrik ist, die das Produkt tatsächlich herstellt. Unsere <a href="/guides/tongkat-ali-drink-supplier-checklist/">Lieferanten-Checkliste</a> enthält genau die Fragen, die zu stellen sind.</li>
        <li><strong>Vereinbaren Sie Startkonditionen schriftlich.</strong> Mindestbestellung, Preisstruktur, Gebiet und welche Marketingunterstützung die Marke bietet. Eine seriöse Marke beantwortet diese Fragen direkt — bei UNI MAX findet dieses Gespräch über WhatsApp statt, mit Antwort am selben Tag.</li>
        <li><strong>Beginnen Sie mit einem Regaltest, skalieren Sie auf Basis von Daten.</strong> Bestellen Sie die kleinste sinnvolle Menge — UNI MAX empfiehlt 12–24 Boxen für eine erste Einzelhandelsposition — platzieren Sie sie in Ihrem besten Kanal und lassen Sie den Abverkauf über Ihre zweite Bestellung entscheiden, nicht den Optimismus.</li>
      </ol>`,
